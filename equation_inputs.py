...
def graph_update(graph_repeat_input=True, graph_axis=equations.Axis(-10, 10, -10, 10)):
    while graph_repeat_input:
        ...
        if equation_input == "POLYNOMIAL" or equation_input == "P":
            ...
            if axis_bool:
                polynomial_user_input = polynomial_input(int(polynomial_degree))
                return ["Polynomial", polynomial_user_input]
            ...
            while not is_float(inputted_radius):
                inputted_radius = input("Enter the radius of the circle >>> ")
            if axis_bool:
                return ["Circle", circle_centre_input(float(circle_x), float(circle_y)), float(inputted_radius)]
            graph_repeat_input = False
            ...
            if axis_bool:
                return ["Trigonometric", trigonometric_input, float(x_coeffcieint_input), float(function_coefficient_input)]
            graph_repeat_input = False
            ...
            if log_input == "LOG":
                ...
                log_base = 0
            if axis_bool:
                return ["Log", log_input, float(log_base)]
            graph_repeat_input = False
        else:
            print("Please enter a valid graph")

def graph_coordinates(type_of_graph, graph_axis):
    if type_of_graph[0] == "Polynomial":
        return equations.Polynomial(graph_axis, type_of_graph[1]).points
    elif type_of_graph[0] == "Circle":
        return equations.Circle(graph_axis, type_of_graph[1], type_of_graph[2]).points
    elif type_of_graph[0] == "Trigonometric":
        return equations.Trigonometric(graph_axis, type_of_graph[1], type_of_graph[2], type_of_graph[3]).points
    elif type_of_graph[0] == "Log":
        return equations.Logarithm(graph_axis, type_of_graph[1], type_of_graph[2]).points